...
local function init()
   ...
   storage.bi_arboretum_table = storage.bi_arboretum_table or {}

   -- Global table for arboretum bases / entries (used throughout the control scripts)
   storage.bi_arboretum_table = storage.bi_arboretum_table or {}
...
local function On_Built(event)
    ...
    if base then
        ...
        if entity.type == "radar" and
			...
			entity.name == entities["bi-arboretum"].hidden[h_key].name and base then
...
local function On_Sector_Scanned(event)
	...
	local radar_unit = event and event.radar and event.radar.unit_number
	local arb_base_id = radar_unit and storage.bi_arboretum_radar_table and storage.bi_arboretum_radar_table[radar_unit]
	if not arb_base_id then
		BioInd.writeDebug("Sector scanned: radar_unit %s not registered in bi_arboretum_radar_table", { tostring(radar_unit) })
		return
	end
	local arb_table = storage.bi_arboretum_table and storage.bi_arboretum_table[arb_base_id]
	if not arb_table then
		BioInd.writeDebug("Sector scanned: storage.bi_arboretum_table[%s] missing", { tostring(arb_base_id) })
		return
	end
